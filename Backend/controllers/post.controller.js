...
export const deletePost = async (req, res) => {
  try {
    ...
    if (post.author.toString() !== userId.toString()) {
      return res
        .status(403)
        .json({ message: "Unauthorized attempt to delete post" });
    }
...
export const createComment = async (req, res) => {
  try {
    ...
    const post = await Post.findByIdAndUpdate(
      ...
      { new: true }
    ).populate("author", "name email username profilePic headline");

    // create a notification if you are not the author of the post
    ...
    try {
      ...
      console.error("Error in sending comment email : ", error.message);
    }
